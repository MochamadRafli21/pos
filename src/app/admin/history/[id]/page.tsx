...
export default async function TransactionDetail({params}:{params:{id:string}}) {
  ...
  const beforeDiscount = orders?.order_items.reduce((total, item) => total + item.price*item.quantity, 0)
  return (
    <main className="min-h-screen w-full md:p-2 flex justify-center overflow-x-hidden">
    <div className='relative w-full min-h-screen md:w-1/3 bg-white'>
    <div className='flex justify-between items-center text-orange-400 p-4 text-xl fill-orange-400 font-bold'>
    <Link href={"/admin/history"}>
    <svg className='w-8 fill-orange-400' clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m22 12.002c0-5.517-4.48-9.997-9.998-9.997-5.517 0-9.997 4.48-9.997 9.997 0 5.518 4.48 9.998 9.997 9.998 5.518 0 9.998-4.48 9.998-9.998zm-8.211-4.843c.141-.108.3-.157.456-.157.389 0 .755.306.755.749v8.501c0 .445-.367.75-.755.75-.157 0-.316-.05-.457-.159-1.554-1.203-4.199-3.252-5.498-4.258-.184-.142-.29-.36-.29-.592 0-.23.107-.449.291-.591z" fill-rule="nonzero"/></svg>
    </Link>
    ...
    <Link href={'/admin/dashboard'} className='mb-2 bg-orange-400 w-full rounded-xl px-4 py-2 flex justify-between'>
    <h1 className="text-white font-bold mx-auto">+ Buat Transaksi Baru</h1>
    </Link>
